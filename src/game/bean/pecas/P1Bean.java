...
// *
// *
public class P1Bean extends Peca {
	
	public P1Bean(BoardBean board) {
		super(board, 1);
	}
	...
	protected boolean desenharZeroGraus(int x, int y) {
		...
		if(board.isPosicaoPreenchida(x, y) || board.isPosicaoPreenchida(x-1, y) ||
				board.isPosicaoPreenchida(x-1, y+1) || board.isPosicaoPreenchida(x-1, y+2))
			return false;
		
		board.preencherPosicao(x, y);
		board.preencherPosicao(x-1, y);
		board.preencherPosicao(x-1, y+1);
		board.preencherPosicao(x-1, y+2);
...
//		board.printBoard();
	...
	protected void apagarZeroGraus(int x, int y) {
		...
		board.esvaziarPosicao(x-1, y+1);
		board.esvaziarPosicao(x-1, y+2);
	}
